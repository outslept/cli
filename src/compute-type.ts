// Copied from https://github.com/antfu/node-modules-inspector/blob/main/packages/node-modules-tools/src/analyze-esm.ts
...
export function analyzePackageModuleType(
  pkgJson: Record<string, any>
): PackageModuleType {
  // We aggressively assume `@types/` are all type-only packages.
  if (pkgJson.name?.startsWith('@types/')) return 'dts';
  ...
  if (esm && cjs) return 'dual';
  if (esm) return 'esm';
  if (fauxEsm) return 'faux';
